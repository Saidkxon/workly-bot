...
public class WorklyTelegramBot implements SpringLongPollingBot {
    ...
    private void handleMessage(Update update, TelegramClient telegramClient) {
        ...
        if (session.getState() == UserState.WAITING_FULL_NAME) {
            ...
            session.setState(UserState.WAITING_DEPARTMENT_SELECTION);
            sendDepartmentSelection(chatId, telegramClient);
            return;
        }

        if (session.getState() == UserState.WAITING_DEPARTMENT_SELECTION) {
            Department department = Department.fromDisplayName(text);
            if (department == null) {
                sendDepartmentSelection(chatId, telegramClient);
                sendPlainMessage(chatId, BotMessages.INVALID_DEPARTMENT_SELECTION, telegramClient);
                return;
            }

            session.setDepartment(department.getDisplayName());
            session.setState(UserState.WAITING_SHIFT_SELECTION);
            sendShiftSelection(chatId, telegramClient);
            return;
        }

        if (session.getState() == UserState.WAITING_SHIFT_SELECTION) {
            Shift shift = Shift.fromDisplayName(text);
            if (shift == null) {
                sendShiftSelection(chatId, telegramClient);
                sendPlainMessage(chatId, BotMessages.INVALID_SHIFT_SELECTION, telegramClient);
                return;
            }

            registerEmployee(telegramUserId, chatId, session.getFullName(), session.getDepartment(), shift, telegramClient);
            resetSession(session);
            return;
        }
        ...
        if (session.getState() == UserState.WAITING_PROFILE_DEPARTMENT_SELECTION) {
            if (employee == null) {
                sendPlainMessage(chatId, BotMessages.NOT_REGISTERED, telegramClient);
                return;
            }
            ...
            session.setRequestedDepartment(department.getDisplayName());
            session.setState(UserState.WAITING_PROFILE_SHIFT_SELECTION);
            sendShiftSelection(chatId, telegramClient);
            return;
        }

        if (session.getState() == UserState.WAITING_PROFILE_SHIFT_SELECTION) {
            if (employee == null) {
                sendPlainMessage(chatId, BotMessages.NOT_REGISTERED, telegramClient);
                return;
            }
            ...
            String message = profileChangeService.createRequest(employee, session.getRequestedDepartment(), shift);
            if (message.startsWith("Bo'lim/smena o'zgartirish so'rovi")) {
                notifyManagersAboutProfileChangeRequest(employee, telegramClient);
                resetSession(session);
                sendMainMenu(employee, chatId, telegramClient, message);
            } else {
                sendPlainMessage(chatId, message, telegramClient);
        ...
        if (session.getState() == UserState.WAITING_ADMIN_BROADCAST_MESSAGE) {
            if (employee == null || employee.getRole() != Role.ADMIN) {
                resetSession(session);
                sendPlainMessage(chatId, "Bu amal faqat adminlar uchun.", telegramClient);
                return;
            }

            if (text.isBlank() || text.length() < 3) {
                sendPlainMessage(chatId, "Xabar juda qisqa. Foydalanuvchilarga yuboriladigan xabarni yozing yoki /cancel yuboring.", telegramClient);
                return;
            }

            int sent = notifyAllActiveEmployees("Admin xabari:\n" + text, telegramClient);
            auditLogService.logAction(AuditActionType.ADMIN_BROADCAST_SENT, employee, null, "Yuborilgan xabarlar soni: " + sent);
            resetSession(session);
            sendMainMenu(employee, chatId, telegramClient, "Xabar " + sent + " ta faol foydalanuvchiga yuborildi.");
            return;
        }
        ...
        if (text.startsWith("/feedback")) {
            String body = text.length() > "/feedback".length() ? text.substring("/feedback".length()).trim() : "";
            if (body.isBlank()) {
                sendPlainMessage(chatId, "Fikringizni yozing. Masalan: /feedback Bot juda qulay.", telegramClient);
            } else {
                feedbackService.save(employee, body);
                sendMainMenu(employee, chatId, telegramClient, "Fikringiz uchun rahmat! Qabul qilindi.");
            }
            return;
        }
        ...
        if (session.getState() == UserState.WAITING_MANAGER_HISTORY_MONTH) {
            ...
            if (targetEmployee == null) {
                sendMainMenu(employee, chatId, telegramClient, "Xodim topilmadi.");
                return;
            }
            sendEmployeeMonthlyExcel(targetEmployee, month, chatId, telegramClient);
            return;
        }
        ...
        if (employee.getRole() == Role.MANAGER || employee.getRole() == Role.ADMIN) {
            ...
            if (text.equals(BotMessages.CMD_MONTH_EXCEL)) {
                try {
                    byte[] fileBytes = excelReportService.buildAllEmployeesSalaryWorkbook(YearMonth.now());
                    if (fileBytes == null) {
                        sendPlainMessage(chatId, BotMessages.EXCEL_EMPTY, telegramClient);
                    } else {
                        sendExcelDocument(chatId, fileBytes, "oylik_maosh_hisoboti.xlsx", BotMessages.EXCEL_CAPTION, telegramClient);
                    }
            ...
            if (text.equals(BotMessages.CMD_SALARIES)) {
                sendPlainMessage(chatId, salaryService.departmentSalariesText(), telegramClient);
                return;
            }

            if (text.startsWith("/setsalary_")) {
                handleSetSalary(text, chatId, telegramClient);
                return;
            }

            if (text.equals(BotMessages.CMD_FEEDBACKS)) {
                sendPlainMessage(chatId, feedbackService.recentFeedbackText(), telegramClient);
                return;
            }

            if (text.equals(BotMessages.CMD_PENDING_PROFILE_CHANGES)) {
                sendPlainMessage(chatId, profileChangeService.getPendingRequestsText(), telegramClient);
                return;
            }

            if (text.equals("/early_dismiss_all")) {
                int count = earlyLeaveService.grantEarlyDismissalForAll(employee.getTelegramUserId());
                if (count < 0) {
                    sendPlainMessage(chatId, "Sizda bu amal uchun huquq yo'q.", telegramClient);
                } else {
                    sendPlainMessage(chatId, count + " ta xodimga bugun erta ketishga ruxsat berildi.", telegramClient);
                    notifyAllActiveEmployees("Bugun erta ketishingizga ruxsat berildi. Ketishni belgilash uchun joylashuvingizni yuboring.", telegramClient);
                }
                return;
            }

            if (text.startsWith("/early_dismiss_")) {
                try {
                    Long targetId = Long.parseLong(text.replace("/early_dismiss_", ""));
                    CorrectionActionResult result = earlyLeaveService.grantEarlyDismissal(employee.getTelegramUserId(), targetId);
                    ...
                    sendPlainMessage(chatId, "Erta yuborish buyrug'i noto'g'ri. Masalan: /early_dismiss_123456789", telegramClient);
                }
                return;
            }
            ...
            if (text.startsWith("/broadcast")) {
                ...
                String body = text.length() > "/broadcast".length() ? text.substring("/broadcast".length()).trim() : "";
                if (body.isBlank()) {
                    sendPlainMessage(chatId, "Xabar matnini yozing. Masalan: /broadcast Ertaga yig'ilish bor.", telegramClient);
                } else {
                    int sent = notifyAllActiveEmployees("Admin xabari:\n" + body, telegramClient);
                    auditLogService.logAction(AuditActionType.ADMIN_BROADCAST_SENT, employee, null, "Yuborilgan xabarlar soni: " + sent);
                    sendPlainMessage(chatId, "Xabar " + sent + " ta faol foydalanuvchiga yuborildi.", telegramClient);
                }
                return;
            }
            ...
            if (text.startsWith("/survey")) {
                ...
                String question = text.length() > "/survey".length() ? text.substring("/survey".length()).trim() : "";
                if (question.isBlank()) {
                    sendPlainMessage(chatId, "Savolni yozing. Masalan: /survey Bot ishidan qoniqasizmi?", telegramClient);
                } else {
                    int sent = notifyAllActiveEmployees("So'rovnoma:\n" + question + "\n\nJavob berish uchun: /feedback javobingiz", telegramClient);
                    auditLogService.logAction(AuditActionType.ADMIN_BROADCAST_SENT, employee, null, "So'rovnoma yuborildi: " + sent);
                    sendPlainMessage(chatId, "So'rovnoma " + sent + " ta faol foydalanuvchiga yuborildi.", telegramClient);
                }
                return;
            }
            ...
            if (text.startsWith("/approve_profile_")) {
                try {
                    Long requestId = Long.parseLong(text.replace("/approve_profile_", ""));
                    CorrectionActionResult result = profileChangeService.approve(requestId, employee.getTelegramUserId());
                    sendPlainMessage(chatId, result.getManagerMessage(), telegramClient);
                    ...
                    sendPlainMessage(chatId, "Profil so'rovini tasdiqlash buyrug'i noto'g'ri.", telegramClient);
                }
                return;
            }

            if (text.startsWith("/reject_profile_")) {
                try {
                    Long requestId = Long.parseLong(text.replace("/reject_profile_", ""));
                    CorrectionActionResult result = profileChangeService.reject(requestId, employee.getTelegramUserId());
                    sendPlainMessage(chatId, result.getManagerMessage(), telegramClient);
                    ...
                    sendPlainMessage(chatId, "Profil so'rovini rad etish buyrug'i noto'g'ri.", telegramClient);
                }
                return;
            }
        ...
        switch (text) {
            ...
            case BotMessages.BUTTON_LEFT_WORK -> {
                if (!attendanceService.canMarkLeaving(employee)) {
                    sendMainMenu(employee, chatId, telegramClient, attendanceService.getLeavingNotAllowedMessage(employee));
                    return;
                }
            ...
            case BotMessages.BUTTON_CHANGE_PROFILE -> {
                session.setState(UserState.WAITING_PROFILE_DEPARTMENT_SELECTION);
                sendDepartmentSelection(chatId, telegramClient);
                return;
            }
            case BotMessages.BUTTON_ADMIN_BROADCAST -> {
                if (employee.getRole() != Role.ADMIN) {
                    sendMainMenu(employee, chatId, telegramClient, "Bu amal faqat adminlar uchun.");
                    return;
                }
                session.setState(UserState.WAITING_ADMIN_BROADCAST_MESSAGE);
                sendPlainMessage(chatId, "Sizning xabaringiz foydalanuvchilar uchun", telegramClient);
                return;
            }
    ...
    private void handleLocation(Update update, TelegramClient telegramClient) {
        ...
        if (session.getState() == UserState.WAITING_ARRIVAL_LOCATION) {
            ...
            if (message.startsWith("Ishga kelish vaqti tasdiqlandi")) {
                message += salaryService.arrivalLatenessNote(employee);
            }
            sendMainMenu(employee, chatId, telegramClient, message);
            return;
        }
    ...
    private void registerEmployee(Long telegramUserId, Long chatId, String fullName, String department, Shift shift, TelegramClient telegramClient) {
    ...
    private void sendDepartmentSelection(Long chatId, TelegramClient telegramClient) {
        List<KeyboardRow> keyboard = new ArrayList<>();
        KeyboardRow row = new KeyboardRow();
        for (Department department : Department.values()) {
            row.add(department.getDisplayName());
            if (row.size() == 2) {
                keyboard.add(row);
                row = new KeyboardRow();
            }
        ...
        sendReplyKeyboard(chatId, BotMessages.ENTER_DEPARTMENT_SELECTION, keyboard, telegramClient);
    }

    private void sendShiftSelection(Long chatId, TelegramClient telegramClient) {
        List<KeyboardRow> keyboard = new ArrayList<>();
        for (Shift shift : Shift.values()) {
            KeyboardRow row = new KeyboardRow();
            row.add(shift.getDisplayName());
            keyboard.add(row);
        }
        sendReplyKeyboard(chatId, BotMessages.ENTER_SHIFT_SELECTION, keyboard, telegramClient);
    }

    private void sendReplyKeyboard(Long chatId, String text, List<KeyboardRow> keyboard, TelegramClient telegramClient) {
        ReplyKeyboardMarkup markup = new ReplyKeyboardMarkup(keyboard, true, true, false, null, false);
        try {
            telegramClient.execute(
                    SendMessage.builder()
                            .chatId(chatId.toString())
            ...
            log.error("Failed to send selection keyboard to chat {}", chatId, e);
        }
    }
    ...
    private void sendEmployeeMonthlyExcel(Employee target, YearMonth month, Long chatId, TelegramClient telegramClient) {
        try {
            byte[] fileBytes = excelReportService.buildEmployeeSalaryWorkbook(target, month);
            sendExcelDocument(chatId, fileBytes, "maosh_hisoboti.xlsx", BotMessages.OWN_EXCEL_CAPTION + " - " + target.getFullName(), telegramClient);
        } catch (RuntimeException exception) {
            log.error("Failed to build employee monthly Excel for chat {}", chatId, exception);
            sendPlainMessage(chatId, BotMessages.EXCEL_SEND_ERROR, telegramClient);
        }
    }

    private void sendExcelDocument(Long chatId, byte[] fileBytes, String fileName, String caption, TelegramClient telegramClient) {
        try {
            InputStream inputStream = new ByteArrayInputStream(fileBytes);
            ...
            log.error("Failed to send Excel document to chat {}", chatId, e);
            sendPlainMessage(chatId, BotMessages.EXCEL_SEND_ERROR, telegramClient);
        }
    }

    private void handleSetSalary(String text, Long chatId, TelegramClient telegramClient) {
        try {
            String payload = text.substring("/setsalary_".length());
            int underscore = payload.lastIndexOf('_');
            if (underscore <= 0) {
                throw new IllegalArgumentException("bad format");
            }
            Department department = Department.valueOf(payload.substring(0, underscore));
            long amount = Long.parseLong(payload.substring(underscore + 1).trim());
            long saved = salaryService.updateMonthlyBase(department, amount);
            sendPlainMessage(chatId, department.getDisplayName() + " uchun yangi fiksa maosh: " + SalaryService.formatSum(saved), telegramClient);
        } catch (Exception e) {
            sendPlainMessage(chatId, "Buyruq noto'g'ri. Masalan: /setsalary_QABUL_BOLIMI_4500000", telegramClient);
        }
    }

    private int notifyAllActiveEmployees(String text, TelegramClient telegramClient) {
        int sent = 0;
        for (Employee employee : employeeRepository.findAllByActiveTrue()) {
            if (employee.getChatId() != null) {
                sendPlainMessage(employee.getChatId(), text, telegramClient);
                sent++;
            }
        }
        return sent;
    ...
    private void resetSession(UserSession session) {
        ...
        session.setRequestedDepartment(null);
    }
    ...
    private String describeTextActivity(UserSession session, String text) {
        if (session == null || session.getState() == UserState.NONE) {
            return text;
        }

        return switch (session.getState()) {
            case WAITING_CORRECTION_DATE -> "Tuzatish sanasini yubordi: " + text;
            case WAITING_CORRECTION_TYPE -> "Tuzatish turini tanladi: " + text;
            case WAITING_CORRECTION_TIME -> "Tuzatish vaqtini yubordi";
    ...
    private String describeLocationActivity(UserSession session) {
        if (session == null) {
            return "Joylashuv yubordi";
        }

        return switch (session.getState()) {
            case WAITING_ARRIVAL_LOCATION -> "Kelish joylashuvini yubordi";
            case WAITING_LEAVING_LOCATION -> "Ketish joylashuvini yubordi";
            default -> "Joylashuv yubordi";
        };
    }
    ...
    private void sendMainMenu(Employee employee, Long chatId, TelegramClient telegramClient, String text) {
        ...
        KeyboardRow rowProfile = new KeyboardRow();
        rowProfile.add(BotMessages.BUTTON_CHANGE_PROFILE);
        ...
        keyboard.add(row2);
        keyboard.add(row3);
        keyboard.add(rowProfile);
        keyboard.add(rowApp);

        if (employee != null && (employee.getRole() == Role.MANAGER || employee.getRole() == Role.ADMIN)) {
            KeyboardRow row4 = new KeyboardRow();
            row4.add(BotMessages.CMD_MONTH_EXCEL);
            row4.add(BotMessages.CMD_EMPLOYEE_HISTORY);

            KeyboardRow row5 = new KeyboardRow();
            row5.add(BotMessages.CMD_PENDING_CORRECTIONS);
            row5.add(BotMessages.CMD_PENDING_PROFILE_CHANGES);

            KeyboardRow row6 = new KeyboardRow();
            row6.add(BotMessages.CMD_PENDING_EARLY_LEAVES);
            row6.add(BotMessages.CMD_PENDING_REGISTRATIONS);

            KeyboardRow row7 = new KeyboardRow();
            row7.add(BotMessages.CMD_EMPLOYEES);
            row7.add(BotMessages.CMD_SALARIES);

            KeyboardRow row8 = new KeyboardRow();
            if (employee.getRole() == Role.ADMIN) {
                row8.add(BotMessages.BUTTON_ADMIN_BROADCAST);
            }
    ...
    private void notifyManagersAboutProfileChangeRequest(Employee employee, TelegramClient telegramClient) {
        var request = profileChangeService.findLatestPendingRequest(employee);
        if (request == null) {
            return;
        }

        String message = "Yangi profil o'zgartirish so'rovi keldi.\n"
                + "Xodim: " + employee.getFullName() + "\n"
                + "Bo'lim: " + request.getCurrentDepartment() + " -> " + request.getRequestedDepartment() + "\n"
                + "Smena: " + Shift.orDefault(request.getCurrentShift()).getDisplayName()
                + " -> " + request.getRequestedShift().getDisplayName() + "\n"
                + "Tasdiqlash: /approve_profile_" + request.getId() + "\n"
                + "Rad etish: /reject_profile_" + request.getId();
        notifyManagers(message, telegramClient);
    }
    ...
    private boolean isMenuOrManagerCommand(String text) {
        ...
        return text.equals(BotMessages.BUTTON_ARRIVED)
                ...
                || text.equals(BotMessages.BUTTON_CHANGE_PROFILE)
                || text.equals(BotMessages.BUTTON_ADMIN_BROADCAST)
                ...
                || text.equals(BotMessages.CMD_ACTIVITIES)
                ...
                || text.startsWith("/approve_profile_")
                || text.startsWith("/reject_profile_")
                || text.startsWith("/approve_")
                || text.startsWith("/reject_")
                || text.equals(BotMessages.CMD_SALARIES)
                || text.equals(BotMessages.CMD_FEEDBACKS)
                || text.equals(BotMessages.CMD_PENDING_PROFILE_CHANGES)
                || text.startsWith("/setsalary_")
                || text.startsWith("/early_dismiss_")
                || text.startsWith("/broadcast")
                || text.startsWith("/survey")
