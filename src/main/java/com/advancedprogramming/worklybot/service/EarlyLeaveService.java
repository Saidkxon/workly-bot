...
public class EarlyLeaveService {
    ...
    // ---- Manager/admin early-dismissal (holiday / early send-home) --------------

    public CorrectionActionResult grantEarlyDismissal(Long actorTelegramUserId, Long targetTelegramUserId) {
        Employee actor = employeeRepository.findByTelegramUserId(actorTelegramUserId).orElse(null);
        Employee target = employeeRepository.findByTelegramUserId(targetTelegramUserId).orElse(null);

        if (actor == null || target == null) {
            return new CorrectionActionResult("Xodim topilmadi.", null, null);
        }
        if (actor.getRole() == Role.EMPLOYEE) {
            return new CorrectionActionResult("Sizda erta yuborish huquqi yo'q.", null, null);
        }
        if (!target.isActive()) {
            return new CorrectionActionResult("Xodim faol emas.", null, null);
        }

        createApprovedDismissal(target, actorTelegramUserId);
        auditLogService.logAction(AuditActionType.EARLY_LEAVE_APPROVED, actor, target, "Erta yuborildi (menejer/admin)");

        return new CorrectionActionResult(
                "Xodim bugun erta ketishi mumkin: " + target.getFullName(),
                target.getChatId(),
                "Sizga bugun erta ketishga ruxsat berildi. Joylashuv yuborib, ketishni belgilashingiz mumkin."
        );
    }

    public int grantEarlyDismissalForAll(Long actorTelegramUserId) {
        Employee actor = employeeRepository.findByTelegramUserId(actorTelegramUserId).orElse(null);
        if (actor == null || actor.getRole() == Role.EMPLOYEE) {
            return -1;
        }

        List<Employee> all = employeeRepository.findAllByActiveTrue();
        for (Employee employee : all) {
            createApprovedDismissal(employee, actorTelegramUserId);
        }
        auditLogService.logAction(AuditActionType.EARLY_LEAVE_APPROVED, actor, null, "Hammaga erta ketishga ruxsat berildi");
        return all.size();
    }

    private void createApprovedDismissal(Employee target, Long actorTelegramUserId) {
        LocalDate today = LocalDate.now(appClock);
        EarlyLeaveRequest request = EarlyLeaveRequest.builder()
                .employee(target)
                .workDate(today)
                .reason("Menejer/admin tomonidan erta yuborildi")
                .status(CorrectionStatus.APPROVED)
                .createdAt(LocalDateTime.now(appClock))
                .reviewedAt(LocalDateTime.now(appClock))
                .reviewedByTelegramUserId(actorTelegramUserId)
                .approvedForTodayLeave(true)
                .build();
        earlyLeaveRequestRepository.save(request);
    }
}
